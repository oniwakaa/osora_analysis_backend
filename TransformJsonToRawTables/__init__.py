...
DOC_FIELD_MAPPING = {
    ...
    "URLDocumento": None,  # Not directly available
...
async def main(myblob: func.InputStream):
    ...
    Processes a JSON blob, transforms it into document and task DataFrames,
    and writes them as Parquet files to Azure Blob Storage.
    
    Args:
        myblob: Azure Function input stream containing JSON data
    """
    ...
    try:
        ...
        try:
            ...
            logging.info(f"Successfully parsed JSON content from blob {myblob.name}")
        ...
        doc_data = extract_document_data(parsed_data, tenant_id, item_id)
        ...
        logging.info(f"Completed processing of blob {myblob.name}")
...
def extract_document_data(data: Dict[str, Any], tenant_id: str, item_id: str) -> Dict[str, Any]:
    """
